...
enum Kind { Empty, Solid, Gravel, Powder, Liquid, Gas, Fire }
...
struct Cell {
    ...
    temp: i16,
    moisture: u8,
    burn: u8, // >0 means the flammable is actively burning; counts down to consumption
    // Overpressure above atmospheric. 0 = ambient, +N = compressed, -N = rarefied.
    // Drives gas dispersion, (future) explosions, and wind coupling.
    pressure: i16,
    // Dissolved solute: what species is in solution in this liquid, and
    // how much (0 = pure, 255 = saturated). Only liquid cells (Water)
    // read/use these; solids and gases keep them at 0. When solute_el ==
    // Element::Derived, solute_derived_id names the compound (so dissolved
    // FeCl and NaCl stay distinguishable in the same water body).
    solute_el: Element,
    solute_amt: u8,
    solute_derived_id: u8,
}
...
impl Cell {
    ...
    #[inline] fn is_frozen(&self)  -> bool { self.flag & Self::FLAG_FROZEN  != 0 }
    ...
    #[inline] fn phase(&self) -> u8 { (self.flag & Self::PHASE_MASK) >> Self::PHASE_SHIFT }
...
struct World {
    cells: Vec<Cell>,
    temp_scratch: Vec<i16>,
    // Scratch buffer for the pressure diffusion pass — we write new values
    // here first so every cell sees the same pre-pass neighbor state.
    pressure_scratch: Vec<i16>,
    // Reusable scratch for tree_support_check (avoid per-frame allocation).
    support_scratch: Vec<bool>,
    support_queue: Vec<(i32, i32)>,
    // Reusable "moved" bitmap for the vacuum tool's direct-pull pass —
    // avoids an 80 KB allocation every frame while the tool is held.
    vacuum_moved: Vec<bool>,
    // "Wind exposure" bitmap. Cells reachable from the open horizontal
    // edges via 4-neighbor non-frozen paths. Frozen walls block exposure,
    // so sealed container interiors are correctly excluded from wind.
    // Rebuilt each step that wind is non-zero.
    wind_exposed: Vec<bool>,
    wind_queue: Vec<(i32, i32)>,
    // Energized-cell bitmap. Cells reachable from any Battery through
    // conductors (plus one extra hop into noble-gas cells for the glow).
    // Rebuilt each step if any Battery exists in the scene.
    energized: Vec<bool>,
    energized_queue: Vec<(i32, i32)>,
    // Circuit voltage — scalar set by the player via the Battery prefab's
    // voltage slider. All active batteries in the scene share this value
    // (MVP simplification). Drives Joule heating: P ∝ V² × resistance.
    battery_voltage: f32,
    // EMF spontaneously generated by a galvanic pair (two dissimilar metals
    // touching the same electrolyte). Derived from the electronegativity
    // gap between the most- and least-reactive metals in contact with
    // brine, recomputed each frame alongside `energized`. Stays at 0 when
    // no galvanic cell is present.
    galvanic_voltage: f32,
    // Voltage in effect this frame: battery slider if a battery pair exists,
    // else galvanic_voltage, else zero. Set by compute_energized.
    active_emf: f32,
    // Electrode masks populated by compute_energized and consumed by
    // electrolysis(). A cell is cathode/anode if it's a metal touching
    // brine that's wired (through non-brine conductors) to the appropriate
    // side of the circuit. Grows naturally as plating deposits new metal
    // on the cathode side.
    cathode_mask: Vec<bool>,
    anode_mask: Vec<bool>,
    // For galvanic mode: which element types act as cathode (high EN) and
    // anode (low EN). None when no galvanic cell is active.
    galvanic_cathode_el: Option<Element>,
    galvanic_anode_el: Option<Element>,
    // Per-cell connected-component size for U (computed once per frame
    // by compute_u_components, read by decay). A U cell's criticality
    // is the size of its contiguous U mass — not local window density —
    // so real critical mass (thousands of atoms) behaves correctly.
    u_component_size: Vec<u16>,
    // Prompt-detonation commit flag. Once a U component crosses critical
    // mass, every cell in it is marked here and stays marked until it
    // transmutes — so the cascade propagates through the entire pile
    // even as the component fragments into Pb and shrinks below the
    // threshold mid-detonation.
    u_burst_committed: Vec<bool>,
    // Per-cell component centroid (written by compute_u_components).
    // Drives the single "ground-zero" shockwave that fires once per
    // component when it first goes critical — distinct from the
    // per-cell burst shockwaves that handle the distributed cascade.
    u_component_cx: Vec<i16>,
    u_component_cy: Vec<i16>,
    // "Central blast has fired for this component" flag. Flood-filled
    // across the whole component when one of its cells first fires the
    // big epicentral wave, so subsequent cells don't re-fire. Reset
    // only when a cell stops being U (transmutes to Pb, gets erased).
    u_central_blast_fired: Vec<bool>,
    frame: u32,
    // Global offset added to every element's ambient_temp, so the user can
    // chill or bake the whole world at will.
    ambient_offset: i16,
    // Gravitational pull magnitude. 1.0 = "Earth-like" (every powder/liquid
    // cell attempts to fall every frame). <1.0 = probabilistic skip per frame
    // (looks like slow-mo / low-G). 0.0 = zero-G (no fall, no buoyancy).
    // Direction is fixed +y; the user explicitly didn't want a direction toggle.
    gravity: f32,
    // Ambient atmospheric oxygen fraction. Default 0.21 matches Earth's
    // real air composition. Empty cells act as if they contain O at this
    // concentration — allows Fe to rust, C to combust, flammables to
    // ignite in "open air" without needing explicit O painted everywhere.
    // 0.0 = vacuum (no O anywhere). >1.0 = oxygen-enriched world, things
    // burn violently.
    ambient_oxygen: f32,
    // Timeline — ring buffer of past grid snapshots for rewind.
    history: Vec<Vec<Cell>>,
    history_write: usize,   // next slot to write
    history_count: usize,   // valid snapshots (capped at HISTORY_CAPACITY)
    rewind_offset: usize,   // 0 = latest, N = N frames back
    // Live shockwaves — impulsive pressure fronts radiating out from
    // detonation points. Spawned by gunpowder ignition and highly
    // exothermic chemistry reactions. Each frame the radius advances,
    // the leading edge ruptures walls / pushes light matter / pressurizes
    // gas, and the wave retires when magnitude falls below threshold.
...
struct Shockwave {
    ...
    cy: f32,
    radius: f32,
    // Original magnitude at origin — i.e. the virtual pressure at r=0.
    // Effective magnitude at the leading edge uses 1/(1+r/r0)^2 falloff.
    yield_p: f32,
}
...
impl World {
    ...
    fn spawn_shockwave(&mut self, cx: i32, cy: i32, yield_p: f32) {
    ...
    fn paint(&mut self, cx: i32, cy: i32, radius: i32, el: Element, derived_id: u8, frozen: bool) {
...
fn window_conf() -> Conf {
    Conf {
        ...
        window_width: 1280,
        window_height: 1024,
        // Fixed window — the grid dimensions are tuned for this exact size
        // so the sim fills the available area with zero dead space. Resize
        // would introduce aspect-ratio gaps we'd have to cover up.
        window_resizable: false,
        // Render at the display's native pixel density so font glyphs
        // aren't blurred through DPI upscaling.
        high_dpi: true,
        ..Default::default()
    }
}
