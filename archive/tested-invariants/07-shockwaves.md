...
- **Pooling monotonicity.** When two nearby shockwaves are merged into a
  single pooled wave, the pooled wave's `yield_p` is ≥ the larger of
  the two inputs. Merging must never *decrease* effective yield.

  *Why:* Kyle's catch. A merge-then-drop bug would silently make
  clustered detonations feel weaker than single ones, which is backwards
  from what players expect.
