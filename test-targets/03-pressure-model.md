...
- **Rupture threshold formula is consistent across paths.** The same
  `BASE + PER_LAYER × (thickness - 1)` formula applies in both the
  pressure-failure path (gas overpressure against a wall) AND the
  shock-failure path (a shockwave's leading edge hitting a wall). If
  the two paths ever use different constants or different thickness
  metrics, walls will behave inconsistently depending on what's trying
  to break them.

  *Why:* Kyle's catch — cross-path consistency is invisible from any
  single test and easy to silently desync during tuning.
...
- **Frozen-material lock.** Frozen cells do not move except via explicit
  rupture (wall burst) or a phase-change unlock (melt, sublimate). No
  pass may silently reposition a frozen cell as an incidental side
  effect.
