...
- **Solute identity resets when `solute_amt` hits 0.** When a cell's
  `solute_amt` drops to zero (solute fully consumed, diluted, or
  reacted away), `solute_el` and `solute_derived_id` must also reset
  to the empty sentinel. A cell with `solute_amt == 0` but a non-empty
  `solute_el` is invalid state and may cause species confusion later.

- **No mixed solutes per water cell.** A water cell never simultaneously
  holds two different solute species. The diffusion and dissolution
  passes must reject attempts to mix — either the incoming species is
  rejected, or the existing solute is displaced/reacted.

  *Why:* Both of the above are Kyle's catches, directly readable from
  the cell layout. They're the kind of drift-prone invariants that
  quietly corrupt derived-compound tracking.

### Derived-compound registry

- **Formula strings are unique.** Registering a compound with a formula
  string equal to an existing entry must reuse that entry's index —
  never duplicate. `DERIVED_COMPOUNDS` should behave as a set keyed by
  formula, not an append-only list.

- **Registry capacity ≤ 256.** `derived_id` is a `u8`, so the registry
  cannot exceed 256 entries. Any registration path that could push
  past this must either gracefully fail or reuse an existing slot.
  Overflow would silently index past the end of the table.
